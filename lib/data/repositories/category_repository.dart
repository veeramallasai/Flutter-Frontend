...
class CategoryRepository {
  ...
  Future<List<CategoryModel>> getCategories() async {
    try {
      final dynamic data =
          (await _apiClient.get('/api/v1/catalog/categories')).data;
      final List<CategoryModel> values = _maps(data)
        ...
        (CategoryModel a, CategoryModel b) =>
            a.sortOrder.compareTo(b.sortOrder),
      );
      return values.isEmpty
          ? localCategories
          : List<CategoryModel>.unmodifiable(values);
    } catch (_) {
      return localCategories;
    }
  }

  CategoryModel _withLocalImage(CategoryModel category) =>
      category.imageUrl.isNotEmpty
          ? category
          : category.copyWith(
            imageUrl: AssetPaths.categoryImage(category.id) ?? '',
          );
...
bool _sameIds(List<CategoryModel> a, List<CategoryModel> b) =>
    a.length == b.length &&
    a.map((CategoryModel item) => item.id).join('|') ==
