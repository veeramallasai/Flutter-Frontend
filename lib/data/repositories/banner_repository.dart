...
class BannerRepository {
  ...
  Future<List<BannerModel>> getBanners() async {
    try {
      final dynamic data =
          (await _apiClient.get('/api/v1/catalog/banners')).data;
      final List<BannerModel> values = _maps(data)
        ...
        (BannerModel a, BannerModel b) => a.priority.compareTo(b.priority),
      );
      return values.isEmpty
          ? localBanners
          : List<BannerModel>.unmodifiable(values);
    } catch (_) {
      return localBanners;
    }
  }
...
bool _sameIds(List<BannerModel> a, List<BannerModel> b) =>
    a.length == b.length &&
    a.map((BannerModel item) => item.id).join('|') ==
